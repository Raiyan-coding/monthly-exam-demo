...
async function loadExamPaper(year, monthIndex, day, subject, examStartUtc, examEndUtc, durationMinutes){
  ...
  // persistent key for this exam instance
  const examKey = paperSeed;

  // If the exam was already submitted, prevent re-showing questions
  const prev = getExamState(examKey);
  if(prev && prev.status === 'submitted'){
    paperInfo.textContent = `This exam was already submitted on ${prev.submittedAt ? new Date(prev.submittedAt).toLocaleString() : 'earlier'}.`;
    questionsList.innerHTML = `<div class="muted">This exam has already been submitted. You cannot retake it.</div>`;
    submitBtn.disabled = true;
    startExamTimer(examStartUtc, examEndUtc);
    return;
  }

  // If an in-progress record exists (previous load), do not show questions after refresh
  if(prev && prev.status === 'in-progress'){
    paperInfo.textContent = `Exam is already in progress — questions are hidden after a refresh. Exam ends at ${new Date(prev.examEndUtc || examEndUtc).toLocaleString()}.`;
    questionsList.innerHTML = `<div class="muted">Exam in progress. The questions are hidden to prevent re-taking. Please wait until the exam ends.</div>`;
    submitBtn.disabled = true;
    // ensure timer shows remaining time and lockdown remains in effect
    startExamTimer(examStartUtc, examEndUtc);
    try{ enableExamLockdown(); }catch(e){}
    return;
  }

  // normal flow: render questions and mark exam as in-progress
  renderQuestions(paper);
  startExamTimer(examStartUtc, examEndUtc);

  // persist that the exam is in progress (prevents reloading to see questions again)
  try{ setExamState(examKey, { status: 'in-progress', startedAt: Date.now(), examEndUtc }); }catch(e){ console.warn('Could not persist exam in-progress state', e); }

  // enable strict lockdown for the duration of the exam
  try{ enableExamLockdown(); }catch(e){ console.warn('Could not enable exam lockdown', e); }
...
async function submitAnswers({auto=false} = {}){
    ...
    try{
      ...
      // always disable lockdown after submit attempt
      try{ disableExamLockdown(); }catch(e){ console.warn('Could not disable lockdown', e); }
...
/* ---------- exam lockdown (prevent copying/devtools/contextmenu during active exam) ---------- */
let _examLockHandlers = [];
function enableExamLockdown(){
  ...
  const preventKey = (e)=>{
    // block common devtools / source-view / copy shortcuts
    const k = e.key.toUpperCase();
    if(k === 'F12') { e.preventDefault(); e.stopPropagation(); }
    if(e.ctrlKey && e.shiftKey && (k==='I' || k==='J' || k==='C' || k==='K')) { e.preventDefault(); e.stopPropagation(); }
    if(e.ctrlKey && k === 'U') { e.preventDefault(); e.stopPropagation(); }
    if(e.ctrlKey && k === 'C') { e.preventDefault(); e.stopPropagation(); }
    if(e.ctrlKey && (k === 'S' || k === 'P')) { e.preventDefault(); e.stopPropagation(); }
  };
  ...
  _examLockHandlers = [ ['contextmenu', preventContext, true], ['selectstart', preventSelect, true], ['copy', preventCopy, true], ['cut', preventCopy, true], ['paste', preventCopy, true], ['keydown', preventKey, true] ];
}
function disableExamLockdown(){
  if(!document.body.classList.contains('exam-lockdown')) return;
  document.body.classList.remove('exam-lockdown');
  for(const h of _examLockHandlers){
    if(h[2]) document.removeEventListener(h[0], h[1], h[2]); else document.removeEventListener(h[0], h[1]);
  }
  _examLockHandlers = [];
}

/* ---------- exam state persistence helpers ---------- */
function _readExamStateMap(){
  try{ return JSON.parse(localStorage.getItem('aq_exam_state') || '{}'); }catch(e){ return {}; }
}
function _writeExamStateMap(map){
  try{ localStorage.setItem('aq_exam_state', JSON.stringify(map)); }catch(e){ console.warn('Could not write exam state', e); }
}
function getExamState(key){ const map = _readExamStateMap(); return map[key]; }
function setExamState(key, obj){ const map = _readExamStateMap(); map[key] = obj; _writeExamStateMap(map); }
function clearExamState(key){ const map = _readExamStateMap(); delete map[key]; _writeExamStateMap(map); }
function markExamSubmitted(key, submissionId){ setExamState(key, { status: 'submitted', submittedAt: Date.now(), submissionId }); }
